...
extension GS where Base: UIScrollView {
    ...
    ///
    /// - Parameters:
    ///   - animator: Refreshable & RefreshAnimatable
    ///   - handler: 回调 closure
    @discardableResult
    ...
    ///
    /// - Parameters:
    ///   - ignoreData: 是否需要重制 无上拉加载 的状态
    ///   - ignoreFooter: 是否需要隐藏 footer
...
open class RefreshHeaderAnimator: UIView, Refreshable, RefreshAnimatable {
    ...
    public required init() {
        ...
        UIView().do { (view) in
            ...
            view.gs.add(iconBGImage) {
                $0.width.equalTo(27)
                $0.height.equalTo(22)
                $0.top.leading.bottom.equalToSuperview()
            }

            view.gs.add(iconImage.then({
            ...
            view.gs.add(indicatorView.then({
                $0.hidesWhenStopped = true
            })) {
                $0.center.equalTo(self.iconBGImage)
            }

            view.gs.add(titleLabel.then({
